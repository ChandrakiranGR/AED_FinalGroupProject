...
public class SupermarketAdminJPanel extends javax.swing.JPanel {
    ...
    // Fetch the order ID from the selected row
    this.orderid = (Integer) source.getModel().getValueAt(row, 0);
    System.out.println("Order ID selected: " + this.orderid);  // Debugging log
 
    // Now fetch the current order object based on the selected order ID
    fetchOrderObject();
 
    // If currentOrder is null, show an error message
    if (this.currentOrder == null) {
        JOptionPane.showMessageDialog(null, "No order selected or order not found.");
        return;  // Exit method to avoid further errors
    }
 
    // Populate the items table with the current order's items
    populateItems();
...
if (this.currentOrder != null) {
        try {
            ...
            System.out.println("Error accepting the order: " + e.getMessage());
            JOptionPane.showMessageDialog(null, "Error accepting the order.");
        }
    } else {
...
if (this.currentOrder != null) {
        try {
            ...
            // Show a confirmation message
            JOptionPane.showMessageDialog(null, "Order has been REJECTED");
 
            // Refresh the orders table to reflect the change
            populateOrders();
        } catch (Exception e) {
            System.out.println("Error rejecting the order: " + e.getMessage());
            JOptionPane.showMessageDialog(null, "Error rejecting the order.");
        }
    } else {
        JOptionPane.showMessageDialog(null, "No order selected.");
    }
...
//        SEND MAIL TO REQUEST PRESCRIPTION
   ...
   public void fetchOrderObject() {
    // Initialize currentOrder to null
    this.currentOrder = null;  // Reset currentOrder before searching
 
    // Search for the order in the customer list
    for (Customer customer : this.network.getCustomerDirectory().getCustomerList()) {
        for (Order o : customer.getOrderlist()) {
            if (o.getOrderId() == this.orderid) {
                this.currentOrder = o;  // Set the current order
                break;  // Exit loop once the order is found
            }
        }
    }
 
    // Check if currentOrder was found
    if (this.currentOrder == null) {
        System.out.println("No order found with ID: " + this.orderid);  // Debugging log
    }
}
